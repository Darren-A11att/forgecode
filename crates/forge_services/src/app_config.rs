...
mod tests {
    ...
    #[tokio::test]
    async fn test_set_default_provider_and_model_atomic() -> anyhow::Result<()> {
        let fixture = MockInfra::new();
        let service =
            ForgeAppConfigService::new(Arc::new(fixture.clone()), ForgeConfig::default());

        // Set both provider and model atomically in a single call
        service
    ...
    #[tokio::test]
    async fn test_set_default_provider_and_model_updates_in_memory_config() -> anyhow::Result<()> {
        let fixture = MockInfra::new();
        let initial_config = ForgeConfig::default();
        let service =
            ForgeAppConfigService::new(Arc::new(fixture.clone()), initial_config);
        ...
        // Verify in-memory config is updated (this is the bug fix verification)
        // Before the fix, this would fail because only the persisted config was updated
        // but the in-memory self.config was not updated
        let provider = service.get_default_provider().await?;
        let model = service.get_provider_model(None).await?;

        assert_eq!(provider, ProviderId::ANTHROPIC);
        assert_eq!(model, ModelId::new("claude-3"));
        Ok(())
    }
    ...
    async fn test_set_default_provider_and_model_then_change_model() -> anyhow::Result<()> {
        ...
        // First, set both provider and model atomically (first-time setup flow)
        service
            .set_default_provider_and_model(
                ProviderId::OPENAI,
                ModelId::new("gpt-4"),
        ...
        // Then change just the model (user running :config-model)
        service
